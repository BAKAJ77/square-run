	...
	resolution(std::clamp(resolution, (uint32_t)30, (uint32_t)230))
{
	...
	const std::string fontPath = Util::GetGameRequisitesDirectory() + "assets/" + fileName.data();
	...
	this->bitmapTexture = TextureBuffer(GL_TEXTURE_2D, GL_RED, totalBitmapWidth, totalBitmapHeight, GL_RED, GL_UNSIGNED_BYTE, nullptr);

	// Fill the texture buffer with the loaded glyph bitmaps
	this->bitmapTexture.BindBuffer();
	...
	for (char glyph = 0; glyph != 127; glyph++)
	{
		...
		this->bitmapTexture.UpdateBuffer(glyphMetrics.textureOffsetX, 0, (uint32_t)glyphMetrics.size.x, 
			(uint32_t)glyphMetrics.size.y, GL_RED, GL_UNSIGNED_BYTE, fontFace->glyph->bitmap.buffer);
	}

	this->bitmapTexture.UnbindBuffer();
	this->bitmapTexture.SetWrappingMode(GL_CLAMP_TO_BORDER, GL_CLAMP_TO_BORDER);
}

Font::Font(Font&& temp) noexcept :
	fileName(temp.fileName), bitmapTexture(std::move(temp.bitmapTexture)), glyphs(temp.glyphs), resolution(temp.resolution)
{}

Font::~Font() = default;

Font& Font::operator=(Font&& temp) noexcept
{
	this->fileName = temp.fileName;
	this->bitmapTexture = std::move(temp.bitmapTexture);
	this->glyphs = temp.glyphs;
...
const TextureBuffer& Font::GetBitmap() const
{
